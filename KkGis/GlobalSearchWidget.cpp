...
GlobalSearchWidget::GlobalSearchWidget(GeoMap *map, QWidget *parent)
    ...
    if (!NLPIR_Init(0, 1)) {
        qWarning() << QStringLiteral("NLPIR��ʼ��ʧ��");
    }
    initRightedMenu();
...
GlobalSearchWidget::~GlobalSearchWidget() {
    if (NLPIR_Exit()) {
        qWarning() << QStringLiteral("�ִ�ϵͳ�˳�ʧ��");
    }
    delete ui;
}
...
void GlobalSearchWidget::initRightedMenu() {
    ...
    connect(translateToFeatureAction, &QAction::triggered, this,
            &GlobalSearchWidget::on_transToFeature_action_triggered);

    ui->resultList->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(ui->resultList, &QListWidget::customContextMenuRequested, this,
            &GlobalSearchWidget::on_fileTree_contextMenu_request);
}