...
void CKLinearRing::CALLBACK vertexCallback(GLvoid *vertex) {}

void CKLinearRing::CALLBACK beginCallback(GLenum which) {
  glBegin(which);
  glColor3d(FR / 255.0, FG / 255.0, FB / 255.0);
}
...
void CKLinearRing::CALLBACK errorCallback(GLenum errorcode) {
  const GLubyte *estring;
  estring = gluErrorString(errorcode);
  exit(0);
}

void CKLinearRing::CALLBACK combineCallback(GLdouble coords[3],
