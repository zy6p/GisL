...
void CKMap::loadSLD(const char *path) {
  ...
  //读取geojson数据
  //    QString gpath="D:\\workstation\\teaching\\course\\oop
  //    gis\\practice\\practise_6\\data\\";
  QString gpath(path);
  ...
  for (int i = 0; i < feats.size(); i++) {
    if (feats[i].isObject() && !feats[i].isNull()) {
      ...
      if (fv["type"].toString().compare("Feature") == 0)
        pFeatures[i] = new CKFeature(feats[i].toObject()); //所有的要素都被存储到对象指针数组pFeatures中，by
                                                           //kmj
    }
  }
}
...
void CKMap::connectFeatSymb() {
  for (int i = 0; i < pRules.size(); i++) {
    for (int j = 0; j < feature_size; j++) {
      if (pFeatures[j]->equalTo(pRules[i]->getField_name(),
                                pRules[i]->getField_value())) {
