...
AttributeTableWidget::AttributeTableWidget(GeoLayer *layer, QWidget *parent)
        : QWidget(parent), layer(layer), originFeatures(layer->getAllFeature()), startRow(-1), endRow(-1),
...
void AttributeTableWidget::initTable() {
    ...
    //table->horizontalHeader()->setSectionResizeMode(QHeaderView::Fixed); //�п���Զ���
...
void AttributeTableWidget::initRightedMenu() {
    ...
    connect(translateToFeatureAction, &QAction::triggered, this,
            &AttributeTableWidget::on_transToFeature_action_triggered);

    table->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(table, &QTableWidget::customContextMenuRequested, this,
            &AttributeTableWidget::on_fileTree_contextMenu_request);
}
...
void AttributeTableWidget::on_glFeatureSelected(GeoFeature *feature) {
    ...
    if (keyPressed == Qt::Key_No) {  //��ʱΪ��ѡģʽ
        startRow = featureRowMap[feature];
        table->clearSelection();
        table->selectRow(startRow);
    }
        /*else if (keyPressed == Qt::Key_Shift) {  //��ʱΪ��ѡģʽ
            if (startRow == -1) {
                startRow = featureRowMap[feature];
                table->clearSelection();
                table->selectRow(startRow);
    ...
    else if (keyPressed == Qt::Key_Control) {  //��Ϊ��ѡģʽ
        startRow = featureRowMap[feature];
        table->selectRow(startRow);
    }
}
