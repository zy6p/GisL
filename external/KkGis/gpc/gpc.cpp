...
void gpc_free_polygon(gpc_polygon *p) {
    ...
    for (c = 0; c < p->num_contours; c++) FREE(p->contour[c].vertex);
    FREE(p->hole);
    FREE(p->contour);
    p->num_contours = 0;
}
...
void gpc_free_tristrip(gpc_tristrip *t) {
    ...
    for (s = 0; s < t->num_strips; s++) FREE(t->strip[s].vertex);
    FREE(t->strip);
    t->num_strips = 0;
}
                ...
                if (contributing) {
                    ...
                    switch (vclass) {
                        ...
                        case ERI:
                            ...
                            cf = NULL;
                            break;
                        case ELI: VERTEX(edge, BELOW, LEFT, xb, yb);
                            edge->outp[ABOVE] = NULL;
                            cf = edge;
                            break;
                        case EMX:
                        ...
                        case IRI:
                            ...
                            edge->outp[ABOVE] = NULL;
                            break;
                        case IMX: VERTEX(edge, BELOW, LEFT, xb, yb);
                            edge->outp[ABOVE] = NULL;
                            cft = IMX;
                            break;
                        case IMM: VERTEX(edge, BELOW, LEFT, xb, yb);
                            edge->outp[ABOVE] = cf->outp[ABOVE];
                            if (xb != cf->xb) VERTEX(cf, ABOVE, RIGHT, xb, yb);
                            cf = edge;
                            break;
                        case EMM: VERTEX(edge, BELOW, RIGHT, xb, yb);
                            edge->outp[ABOVE] = NULL;
                            new_tristrip(&tlist, edge, xb, yb);
                            cf = edge;
                            break;
                ...
                if ((e0->bundle[ABOVE][CLIP] || e0->bundle[ABOVE][SUBJ])
                    ...
                    switch (vclass) {
                        ...
                        case EMX:
                            ...
                            break;
                        case IMN: P_EDGE(prev_edge, e0, ABOVE, px, iy);
                            VERTEX(prev_edge, ABOVE, LEFT, px, iy);
                            N_EDGE(next_edge, e1, ABOVE, nx, iy);
                            VERTEX(next_edge, ABOVE, RIGHT, nx, iy);
